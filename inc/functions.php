...
function cpfp_register_post_type_page_settings() {
	...
	foreach ( $post_types as $post_type ) {
		...
		register_setting( 'cpfp_custom_post_type_page_for_posts', 'cpt_page_for_posts_override_slug_' . $post_type->name );
	}
...
function cpfp_display_post_type_page_fields( $post_types ) { ?>
	<div id="cpt_archive_pages_metabox" class="postbox" style="margin-bottom: 50px">
		...
		<div class="inside">
			<p>Select a Page to act as the archive page for each custom post type. Only public post types with an <code>has_archive</code>
				enabled will be displayed here.</p>
			...
			if ( $post_types ) {
				foreach ( $post_types as $post_type ) {
					...
					$override_slug_option = 'cpt_page_for_posts_override_slug_' . $post_type;
					$override_enabled = get_option( $override_slug_option, false );
						...
						<tr>
							...
							<td>
								<select id="<?php echo esc_attr( $option_name ); ?>"
									...
									<option
										value="0" <?php selected( $current_value, 0 ); ?>><?php esc_html_e( 'None (Default)', 'cpt-page-for-posts' ); ?></option>
									<?php
									foreach ( $pages as $page ) :
										...
										?>
										<option
											value="<?php echo esc_attr( $page_id ); ?>" <?php selected( $current_value, $page_id ); ?>><?php echo esc_html( $page->post_title ); ?></option>
									<?php endforeach; ?>
								</select>

								<p style="margin-top: 10px;">
									<label>
										<input type="checkbox" name="<?php echo esc_attr( $override_slug_option ); ?>"
										       value="1" <?php checked( $override_enabled, 1 ); ?>>
										<?php esc_html_e( 'Use page slug as archive slug.', 'cpt-page-for-posts' ); ?>
									</label>
								</p>
							</td>
						</tr>
		...
		<tr>
			...
			<td>
				<input type="checkbox" id="<?php echo esc_attr( $option_name ); ?>"
				       name="<?php echo esc_attr( $option_name ); ?>" value="1" <?php checked( $enabled, 1 ); ?>>
...
function custom_post_type_page_uninstall() {
	...
	if ( $enable_cleanup ) {
		...
		foreach ( $post_types as $post_type ) {
			...
			$override_option_name = 'cpt_page_for_posts_override_slug_' . $post_type->name;
	...
	// Delete the uninstall cleanup option itself
	delete_option( 'cpt_page_for_posts_enable_uninstall_cleanup' );

	// Flush rewrite rules to remove any custom post type archive slugs
	flush_rewrite_rules();
}
...
function cpfp_get_page_slug_path( $page_id ) {
	$parts = [];

	while ( $page_id && $page = get_post( $page_id ) ) {
...
add_filter( 'register_post_type_args', function ( $args, $post_type ) {
	$override = get_option( 'cpt_page_for_posts_override_slug_' . $post_type );
	$linked_page_id = get_option( 'cpt_page_for_posts_' . $post_type );

	if ( $override && $linked_page_id ) {
		$slug_path = cpfp_get_page_slug_path( $linked_page_id );
		if ( $slug_path ) {
			$args['has_archive'] = $slug_path;
		}
	}
...
add_action( 'admin_init', function () {
	if (
		...
		$_POST['option_page'] === 'cpfp_custom_post_type_page_for_posts' &&
